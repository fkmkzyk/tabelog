...
- **訪問日時の自動抽出（EXIF）**:
  ...
  - 訪問日時はAIプロンプトに「6月中旬の夜（ディナー）」のような曖昧化した形式で渡され（第13フェーズ参照。具体的な日付は渡さない）、季節や時間帯を踏まえた自然なレビュー生成に活用されます。
...
### 第17フェーズ: 画面設計ブラッシュアップ第3弾（操作精度とローディング）
1. **評価スライダーの −/＋ ステッパー**: 作成フォームのスライダー両脇に0.2刻みの−/＋ボタンを追加し、指先での微調整を正確に。上限・下限では自動で無効化。狭い画面では星表示を1行目に独立させ、−/スライダー/＋/数値を2行目に整列。
2. **スケルトンローディング**: 一覧取得中のスピナーを、ダイジェストカード形のスケルトン（シマーアニメーション付き・3枚）に置き換え、読み込み後のレイアウトのガタつきを解消。`prefers-reduced-motion` ではアニメーションを停止。
